...
with tempfile.TemporaryDirectory(prefix="tauceti-continuation-") as td:
    ...
    opts = wu.RoundOpts(
        only=["fix"], agent="codex", work_model="codex", sandbox_host=True, dry_run=False,
    ...
    def author(_cwd, _prompt, _profile, _logdir):
        ...
        check("second attempt sees staged bytes", git(checkout, "show", ":tracked.txt", capture=True).stdout, "base\nstaged\n")
        ...
        check("second attempt reuses build artifact", (checkout / "build" / "artifact.bin").read_bytes(), b"built-once\x00")
    ...
    try:
        ...
        check("first attempt leaves mixed index/worktree", git(checkout, "status", "--porcelain", capture=True).stdout.splitlines(), ["MM tracked.txt", "?? new.txt"])
        ...
        meta_path.write_text(json.dumps({
            "pr": 77, "public_head": public, "candidate_head": public,
            "commit_ref": commit_ref, "stash_ref": None, "stage": "fix",
        }))
        ...
        fresh_path.write_text(json.dumps({
            "pr": 77, "public_head": local, "candidate_head": local,
            "commit_ref": fresh_ref, "stash_ref": None, "stage": "fix",
        }))
        ...
        check("rc0 refused publication retains candidate", git(checkout, "rev-parse", "HEAD", capture=True).stdout.strip(), local)
        ...
        for raw in ("[]", "null"):
            ...
            check(f"non-object recovery {raw} fails closed", wu._do_fixlike(
                worker, survey, candidate, opts, False, prompt_file="fix.md", label="fix"
            ), 1)
        ...
        check("legacy restore preserves index", git(checkout, "diff", "--cached", "--name-only", capture=True).stdout, "tracked.txt\n")
        ...
        check("ordinary preparation retains ignored artifact", (checkout / "build" / "artifact.bin").read_bytes(), b"built-once\x00")
        git(checkout, "switch", "topic")
        git(checkout, "branch", "-f", "main", "topic")
        main_head = git(checkout, "rev-parse", "HEAD", capture=True).stdout.strip()
        check("ahead local main blocks destructive preparation", agents.continuation_checkout(cfg, "missing", "c" * 40), None)
        check("ahead local main remains referenced", git(checkout, "rev-parse", "main", capture=True).stdout.strip(), main_head)
