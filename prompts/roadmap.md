...
- **Admit the target before authoring.** Write down, for yourself and in the PR body, the exact roadmap item and milestone, the declarations (or files and named declarations) that milestone expects, and one concrete consumer in the current repository or in a later milestone explicitly named by the roadmap. Read the consumer's surrounding code and confirm that this PR supplies something it can use. A declaration with no identified consumer, a duplicate of an existing declaration, or a capability that is merely related to the roadmap is not an admitted target: choose another step or stop without a PR. Re-check `main` and the open sibling PRs immediately before editing; an item that became implemented or claimed is no longer admitted.
